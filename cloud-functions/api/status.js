  ...
  try{
    ...
    const probeResults = await Promise.all(probeTargets.map(async ([domain, url]) => {
      ...
      try {
        ...
        return { domain, ok: probe.status >= 200 && probe.status < 400, status: probe.status, latency_ms: elapsed, error: '' };
      } catch (error) {
        return { domain, ok: false, status: 0, latency_ms: Date.now() - started, error: error && error.name ? String(error.name).slice(0, 40) : 'fetch_error' };
      }
    }));
